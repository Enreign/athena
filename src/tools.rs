...
fn claude_code_nested_session_conflict_output() -> String {
...
impl Tool for ClaudeCodeTool {
    ...
    async fn execute(&self, _session: &DockerSession, params: &Value) -> Result<ToolResult> {
        if std::env::var_os("CLAUDECODE").is_some() {
            return Ok(ToolResult {
                success: false,
                output: claude_code_nested_session_conflict_output(),
            });
        }
...
mod tests {
    ...
    #[test]
    fn test_claude_code_nested_session_conflict_emits_contract_marker() {
        let out = claude_code_nested_session_conflict_output();
        assert!(out.starts_with("[athena_cli_contract]"));
        assert!(out.contains("tool=claude_code"));
        assert!(out.contains("code=session_conflict"));
        assert!(out.contains("retry_same=false"));
        assert!(out.contains("fallback=true"));
    }
